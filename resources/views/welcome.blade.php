<!DOCTYPE html>
<html lang="es">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">

        <title>Laravel</title>
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">

        <!-- Fonts -->
        <link href="https://fonts.googleapis.com/css?family=Nunito:200,600" rel="stylesheet">

        <!-- Styles -->
        <style>
            html, body {
                background-color: #fff;
                color: #636b6f;
                font-family: 'Nunito', sans-serif;
                font-weight: 200;
                height: 100vh;
                margin: 0;
            }

            .full-height {
                height: 100vh;
            }

            .flex-center {
                align-items: center;
                display: flex;
                justify-content: center;
            }

            .position-ref {
                position: relative;
            }

            .top-right {
                position: absolute;
                right: 10px;
                top: 18px;
            }

            .content {
                text-align: center;
            }

            .title {
                font-size: 84px;
            }

            .links > a {
                color: #636b6f;
                padding: 0 25px;
                font-size: 13px;
                font-weight: 600;
                letter-spacing: .1rem;
                text-decoration: none;
                text-transform: uppercase;
            }

            .m-b-md {
                margin-bottom: 30px;
            }
        </style>
    </head>
    <body>
        <div class="flex-center position-ref ">
            <div class="content">
                <div id="dutylist" class="title m-b-md">
                </div>
            </div>
            <div className="card-header">Bienvenido a Reportes de Recetas de Comida</div>
            <div className="card-body">Seleccione el reporte que decea ver</div>
        </div>

        <p>
            <a href="{{ route('departamento') }}" class="btn btn-sm btn-primary">
                Visualizar Lista de Platos por departamento
            </a>
        </p>
        <p>
            <a href="{{ route('plato') }}" class="btn btn-sm btn-primary">
                Visualizar Lista de Cantidad de Ingredientes por plato
            </a>
        </p>
        <p>
            <a href="{{ route('departamentos.pdf') }}" class="btn btn-sm btn-primary">
                Descargar Lista de Platos por departamento
            </a>
        </p>
        <p>
            <a href="{{ route('platos.pdf') }}" class="btn btn-sm btn-primary">
                Descargar Lista de Cantidad de Ingredientes por plato
            </a>
        </p>
    </body>
    <script type="text/javascript" src="js.app.js"></script>
</html>
